...
pub(crate) fn dialog_gcd_body_carry_trunc_width(active_width: usize, step: usize) -> usize {
    ...
    w = w.saturating_add(dialog_gcd_early_body_notch_extra_for_step(step));
...
/// Optional research-only per-step body-carry notch schedule, encoded as
/// `step:extra,step:extra,...`. This narrows only the materialized sub/add BODY
/// carry band, not the global active-width used by cswap/comparator/shift.
pub(crate) fn dialog_gcd_early_body_notch_extra_for_step(step: usize) -> usize {
    let Ok(spec) = std::env::var("DIALOG_GCD_EARLY_BODY_NOTCHES") else {
        return 0;
    };
    spec.split(',')
        .filter_map(|tok| {
            let (s, e) = tok.trim().split_once(':')?;
            Some((s.trim().parse::<usize>().ok()?, e.trim().parse::<usize>().ok()?))
        })
        .filter(|(s, _)| *s == step)
        .map(|(_, e)| e)
        .max()
