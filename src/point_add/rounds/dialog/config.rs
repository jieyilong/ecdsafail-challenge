...
pub(crate) fn dialog_gcd_apply_intermediate_lowq_enabled() -> bool {
    std::env::var("DIALOG_GCD_APPLY_INTERMEDIATE_LOWQ")
        .ok()
        .as_deref()
        == Some("1")
}
...
pub(crate) fn dialog_gcd_apply_boundary_free_after_hmr_enabled() -> bool {
    std::env::var("DIALOG_GCD_APPLY_BOUNDARY_FREE_AFTER_HMR")
        .ok()
        .as_deref()
        == Some("1")
}
