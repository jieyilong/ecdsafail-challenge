...
pub(crate) fn cuccaro_add_fast_borrowed_carries_gate_suffix(
    b: &mut B,
    a: &[QubitId],
    acc: &[QubitId],
    c_in: QubitId,
    ...
    if n == 1 || gate_suffix == 0 {
        return cuccaro_add_fast_borrowed_carries(b, a, acc, c_in, carries);
    }
    let borrowed = (n - 1).saturating_sub(gate_suffix.min(n - 1));
    if borrowed >= n - 1 {
        return cuccaro_add_fast_borrowed_carries(b, a, acc, c_in, carries);
    }
    ...
    for i in (borrowed.max(1)..n - 1).rev() {
...
pub(crate) fn cuccaro_add_fast_low_to_ext_borrowed_carries_gate_suffix(
    b: &mut B,
    a: &[QubitId],
    acc_ext: &[QubitId],
    c_in: QubitId,
    ...
    let borrowed = n.saturating_sub(gate_suffix.min(n - 1));
    assert!(borrowed >= 1);
    assert!(carries.len() >= borrowed);
...
pub(crate) fn cuccaro_sub_fast_low_to_ext_borrowed_carries_gate_suffix(
    b: &mut B,
    a: &[QubitId],
    acc_ext: &[QubitId],
    c_in: QubitId,
...
pub(crate) fn cuccaro_sub_fast_borrowed_carries_gate_suffix(
    b: &mut B,
    a: &[QubitId],
    acc: &[QubitId],
    c_in: QubitId,
    ...
    if n == 1 || gate_suffix == 0 {
        return cuccaro_sub_fast_borrowed_carries(b, a, acc, c_in, carries);
    }
    let borrowed = (n - 1).saturating_sub(gate_suffix.min(n - 1));
    if borrowed >= n - 1 {
        return cuccaro_sub_fast_borrowed_carries(b, a, acc, c_in, carries);
