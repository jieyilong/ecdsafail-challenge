...
Centered Euclid improves the relaxed ledger: payload p99/max `336/341`, count p99 `118`, one-boundary scratch p99 `710`, weighted extraction p99 `43,935`, projected point-add `2,443,100` (`-556,900` to 3M). Fixed scan is still dead (`+28,970,172` at 1 CCX/static trial). A first packed-extractor note overestimated alignment room by forgetting the forward+reverse denominator pass; corrected one-way extraction budget is `490,705`, leaving `~1,716` CCX/quotient for alignment after compare/sub and leading-scan floors. A generic `n log n` barrel at `2048`/quotient would miss by `+156,860` point-add CCX. So centered Euclid is the better relaxed-2800q candidate, but it remains speculative until a reversible extractor scales with weighted quotient payload, uses cheaper offset/metadata alignment rather than fixed scans or stock barrels, and still provides exact boundary cleanup and phase-clean denominator reverse.
