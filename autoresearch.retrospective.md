...
Centered Euclid improves the relaxed ledger: payload p99/max `336/341`, count p99 `118`, one-boundary scratch p99 `710`, weighted extraction p99 `43,935`, projected point-add `2,443,100` (`-556,900` to 3M). Fixed scan is still dead (`+28,970,172` at 1 CCX/static trial), but the packed extractor has `~5,874` CCX/quotient for alignment after compare/sub and leading-scan floors. So centered Euclid is the better relaxed-2800q candidate, but it remains speculative until a reversible extractor scales with weighted quotient payload, uses offset/metadata alignment rather than fixed scans, and still provides exact boundary cleanup and phase-clean denominator reverse.
