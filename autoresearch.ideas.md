...
- **Architecture pivot, not micro-tuning:** stop spending time on Kaliski thresholds/local Solinas. New BY selector test `streaming_limb_selector_is_exact_but_state_heavy` proves a no-full-denominator 16-bit limb recurrence `(f_j,g_j)=A_j*(p>>16j,x>>16j)+c_j` can generate all 35 branch windows exactly with 12 limbs per `A/c` entry, but the naive state is `6*12*16=1152` bits (8 limbs fails on all sampled denominators). This is a constructive selector architecture but not yet Google-qubit-shaped; next work is compress/recompute `A_j` from pattern/matrix history or factor it via the Hermite window structure. See `src/point_add/by_sota_architecture.md`.
