...
- **BY compact denominator-pair history sink is state-good but raw-divstep gate-dead.** `denominator_pair_plus_50_sidecar_can_hold_raw_history_on_samples` / `denominator_pair_fixed_slack_schedule_50_sidecar_on_samples` show that the ordinary 256-bit BY denominator pair `(f,g)` can absorb raw branch history into high-zero slack with a 50-bit sidecar over 8192 samples (selector/history≈564 bits incl. signs). But `tapered_2adic_branch_generator_cost_is_still_too_high` invalidates raw per-bit use: compute=1,004,080 CCX, compute+uncompute=2,008,160 CCX, peak=3372q, projected total with one replay+scaffold≈3.79M. Only remaining BY version worth touching is compact pair/history + selected fixed-matrix window update (`tapered_fixed_matrix_denominator_budget_is_sota_shaped_if_selection_solved`: compute+uncompute≈607,657 CCX, but current model peak=3424q). Hard gate: no full BY point-add wiring unless a single-window selected fixed-matrix + history allocator prototype is <~10k CCX/window and qubit-shaped. Full-ratio remains gate-blocked (`h0` constmul okay; A-step dense; serial inverse proxy~7.31M). See `src/point_add/by_sota_architecture.md`.
