...
- **New best BY selector route: compact denominator-pair history sink.** `denominator_pair_plus_49_sidecar_can_hold_raw_history_on_samples` shows that the ordinary 256-bit BY denominator pair `(f,g)` can absorb raw branch history into its high-zero slack with only a 49-bit sidecar over 1024 sampled secp256k1 denominators (`max_converge=558`, worst deficit=49). Including sign bits, selector/history is about `512+2+49=563` bits, inside the ~600 low-qubit allowance, and uses linear shift/add divsteps instead of full-ratio A-step inverses. This supersedes full-ratio as the most promising path. Next: prove/enforce the 49-bit bound, design a reversible allocator/compactor for history bits as `bitlen(f)+bitlen(g)` changes, and couple raw bits to scaled replay/reverse cleanup. Full-ratio remains a state-optimal but gate-blocked fallback (`h0` constmul okay at ~221k--443k; A-step dense; serial inverse proxy ~7.31M; W=32 still low valuation). See `src/point_add/by_sota_architecture.md`.
