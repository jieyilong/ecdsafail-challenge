...
- `reversible_pattern_delta_decoder_matches_and_cleans` upgrades the BY A-control decoder from budget to circuit: one 16-step window `(pattern,delta_start,A=0)->(pattern,delta_next,A_mask)` costs 1,808 CCX forward / 3,616 roundtrip, peak 68q, exact and phase-clean. 35 forward decodes≈63k, within the 150k integration margin. Scheduling issue remains: inverse clears A but restores delta_start, so production replay needs a delta pebble, reverse-from-delta_next A-clearer, or on-the-fly A use inside self-cleaning windows.
